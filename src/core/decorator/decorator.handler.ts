...
const ReflectDetector = (path?: string, method: string = HttpMethods.GET, target?: any, propertyKey?: any): void => {
  ...
  const routes = Reflect.getMetadata(MetadataKeys.ROUTERS, target.constructor) as Array<RouteDefinition>;
  routes.push({
  ...
  console.log(Reflect.getMetadata(MetadataKeys.ROUTERS, target.constructor) as Array<RouteDefinition>)
}
